...
```
npm/
  aval/                 the package people depend on
    package.json.in      optionalDependencies on all six platform packages
    bin/aval.js         resolves the platform binary and execs it
    README.md
  platform/
...
- `npm ci --ignore-scripts` is a normal hardening choice and a security default
  ...
  no binary in it, and the failure surfaces later as "aval: not found" from a
  git hook.
...
## Why `bin/aval.js` is not on the hook path
...
It never pays that. `aval init` bakes `current_exe()`, which is the **native**
...
`crates/aval/tests/init.rs` is what keeps that true.
